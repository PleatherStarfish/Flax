...
let Oscillator = (props) => {
    ...
    const objectId = props.keyId;

    return (
        <Draggable bounds="parent" onDrag={props.handleDrag} {...props.dragHandlers} onClick={(e) => props.deleteObject(e)}>
            <div id={props.keyId} className="oscillator-box" style={{backgroundColor: props.color, borderColor: props.color, cursor: "grab", position: "absolute"}}>
                <div id={`container-${props.keyId}`} className={"d-flex justify-content-between"}>
                    <h1 style={{color: "grey"}}>Oscillator</h1>
                    <div id={`delete-${props.keyId}`} style={{cursor: "pointer"}}>
